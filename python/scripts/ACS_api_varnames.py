...
page = requests.get('http://api.census.gov/data/2015/acs5/variables.html')
...
ACS.to_csv('../datasets/housing-insights/ACS_api_vars.csv', header=False, encoding='utf-8')

ACS = pd.read_csv('../datasets/housing-insights/ACS_api_vars.csv', low_memory=False, encoding='utf-8')
...
import sqlite3
connection = sqlite3.connect('../datasets/housing-insights/ACS_api_vars.db.sqlite')
...
poverty = Q('SELECT * FROM ACS WHERE Label LIKE "%below poverty%"')
...
labor = Q('SELECT * FROM ACS WHERE Label LIKE "In labor force%"')
